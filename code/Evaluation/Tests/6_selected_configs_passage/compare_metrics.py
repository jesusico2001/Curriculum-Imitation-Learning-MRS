...
def main():
    path_saves = os.path.dirname(os.path.abspath(__file__))+"/results/metrics_evo/small/"
    ...
    # metrics = ["n_task_completed"]
    numAgents = [6, 12]
    noise_levels = [0, 0.1, 0.25]

    # os.makedirs(path_saves+"l2_outliers", exist_ok=True)
    # for noise in noise_levels:
    #     for na in numAgents:
    #         plotL2_in_out(noise, na)
    #         # plt.savefig(path_saves+"l2_outliers/"+str(na)+"robots_"+str(noise)+".png")   
    #         # plt.cla()  
    #         plt.show() 


    colors = plt.get_cmap("viridis", len(numAgents)+1)
    for metric in metrics:
        for noise in noise_levels:
            ...
            # plt.title(metric+" evolution - noise="+str(noise)+" | Difficulty:"+str(difficulty), fontsize=25)
            plt.yscale('log')
            plt.xlabel('Iterations', fontsize=30)
            plt.ylabel(metric_name[metric], fontsize=30)
            plt.xticks(fontsize=25)
            plt.yticks(fontsize=25)
            plt.legend(fontsize=25)
            plt.grid(True)
            plt.savefig(path_saves+metric+"_noise"+str(noise)+".png")   
            plt.cla()  
            # plt.show() 
