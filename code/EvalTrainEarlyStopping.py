...
def main(control_policy='FS', architecture='LEMURS', nAttLayers=3, epochs=40000, numTrain=20000, numVal=20000, numTests=20000, 
    ...
    epoch_stop = val_epochs[-1]

    print("Real epochs obtained = ", epoch_stop)
    ...
    # Load best model from early_Stopping
    myLearnSystem.load_state_dict(torch.load(path_checkpoints+"/epoch_"+str(epoch_stop)+".pth", map_location=device))
    with torch.no_grad():
        learned_trajectories = myLearnSystem.forward(initial_states.to(device), simulation_time, step_size)
    print("\nCheckpoint loaded for evaluation: ", epoch_stop)

    # Plot qualitative
    plt.clf()
    plt.grid(False)
    plotFrame(learned_trajectories[:,0,:].to(device), real_trajectories[:,0,:].to(device), epochs, epoch_stop, maxNumSamples, numAgents, numSamplesLog)
    ...
    with open(path_results+"/info.txt", 'w') as file:
        text = "Best epoch: "+str(epoch_stop)+"\n" + "L2 loss: "+str(float(loss_test)) + "\nAvg dist error: "\
