...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.agent_spacing = self.agent_radius / math.sin(math.pi / self.n_agents) * 1.2
        self.agent_angular_spacing = 2 * math.pi / (self.n_agents-1)
        self.passage_width = 0.5
        self.passage_length = self.x_semidim / 5.0
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        for i, goal in enumerate(goals):
            if i == self.n_agents - 1:
                ...
                angle = i * self.agent_angular_spacing
                offset = torch.tensor(
                    [
                        [self.agent_spacing * math.cos(angle), self.agent_spacing * math.sin(angle)]
                    ],
                    device=self.world.device,
                    dtype=torch.float32,
                )
