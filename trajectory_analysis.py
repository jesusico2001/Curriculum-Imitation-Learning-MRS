...
def plotFrame(my_learned_trajectory, real_trajectory, nEpochs, epoch_save, maxNumSamples, numAgents, numSamplesLog):
    # Plotting
    plt.rcParams.update({'font.size': 20})

    if len(numSamplesLog) > 0 :
        i = len(numSamplesLog)-1
...
def updateFrame(frame, myLearnSystem, initial_state, real_trajectory, nEpochs, maxNumSamples, 
                numAgents, device, simulation_time, step_size, path_checkpoints, numSamplesLog):
    ...
    try:
        ...
        plt.clf()
        plotFrame(my_learned_trajectory, real_trajectory, nEpochs, epoch_save, maxNumSamples, numAgents, numSamplesLog)
